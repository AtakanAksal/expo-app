...
const SearchAndFilterComponant = ({setListItems, listItems, filteredData, setFilteredData, queryString }) =>{
    ...
    const contains = (data, query) => {
      ...
      //  const strQuery=`${singledata}${queryString}${aaaa}`
          return contains(singledata?.sender_user.first_name, text); //         `${singledata}.${queryString}`
        });
        setFilteredData(filtered);    
    }
      else {setFilteredData(listItems); } 
      };
    ...
    return(
        ...
        <Image style={{ height:25, width:25, paddingVertical:8, marginLeft:10, marginRight:5}} source={Arama} resizeMode="contain" />  
        <TextInput placeholder="Hızlı Arama"
...
export default SearchAndFilterComponant;

const styles = StyleSheet.create({
  
   
      searchAndFilterComponant : {
          flexDirection: 'row',  marginVertical:10, width:'100%', height:HEIGHT/16,
...
const Filter = () =>{
    return(
        ...
        <Image style={{ height:25, width:25, marginVertical:8,  marginRight:5}} source={FilterImg} resizeMode="contain" />  
