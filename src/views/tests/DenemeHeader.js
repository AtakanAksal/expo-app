...
import { Image, StyleSheet, View, Text, Dimensions, Animated } from "react-native";
...
const CollapsibleTabView = () => {
  ...
  const renderHeader = () => {
    ...
    const transY = Animated.diffClamp(clampedScrollY, -HeaderHeight, 0);
    return (
      <Animated.View style={[styles.header, { transform: [{ translateY: transY }] }]}>
        <Text>{"Header"}</Text>
      </Animated.View>
    );
  };
  ...
  const renderTabBar = (props) => {
    ...
    const clampedScrollY = scrollY.interpolate({
      inputRange: [0, TabBarHeight + TabBarHeight],
      outputRange: [TabBarHeight + TabBarHeight, 0],
    });
    // const minusScrollY = Animated.multiply(clampedScrollY,-2);
    const transY = Animated.diffClamp(clampedScrollY, 0, TabBarHeight);
    return (
      <Animated.View
        style={{
          top: 0,
      ...
      >
        <TabBar
          ...
          renderIcon={({ route, focused, color }) => (
            <Image style={styles.img} source={HomeIcon} resizeMode="contain" />
          )}
          */
        />
      </Animated.View>
